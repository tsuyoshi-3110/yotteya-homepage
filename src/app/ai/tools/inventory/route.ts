...
// 役割: 在庫検索ツールAPI（AI からのツール呼び出し用）
// 入力: { siteKey: string, query?: string, topN?: number }
// 出力: { items: InventoryRow[], passages: string[], generatedAt: string }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const items = typeof query === "string" && query.trim()
